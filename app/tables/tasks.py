...
def make_default_log_daily(user_pk):
    date_range = [datetime.date.today().replace(day=1) + datetime.timedelta(i) for i in range(0, calendar.monthrange(datetime.date.today().year, datetime.date.today().month)[1])]
    for date in date_range:
        for time in ['아침', '점심', '저녁', '간식(오전)', '간식(오후)']:
